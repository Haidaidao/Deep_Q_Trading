...
class DeepQTrading:
    ...
    def run(self):
        ...
        for i in range(0,numFile):
            trainMinLimit = getDate_Index(weeksFrame, datesFrame, trainMin[i])
            trainMaxLimit = getDate_Index(weeksFrame, datesFrame, trainMax[i])

            validMinLimit = getDate_Index(weeksFrame, datesFrame, validMin[i])
            validMaxLimit = getDate_Index(weeksFrame, datesFrame, validMax[i])
        
            testMinLimit = getDate_Index(weeksFrame, datesFrame, testMin[i])
            testMaxLimit = getDate_Index(weeksFrame, datesFrame, testMax[i])

            train = TrendSlope(iteration = i, minLimit=trainMinLimit,maxLimit=trainMaxLimit, name = "Week" ,type = "train")
            train.writeFile()
            valid = TrendSlope(iteration = i, minLimit=validMinLimit,maxLimit=validMaxLimit, name = "Week" ,type = "valid")
            valid.writeFile()
            test  = TrendSlope(iteration = i, minLimit=testMinLimit,maxLimit=testMaxLimit, name = "Week" ,type = "test")
            test.writeFile()   

            trainMinLimit = getDate_Index(daysFrame, datesFrame, trainMin[i])
            trainMaxLimit = getDate_Index(daysFrame, datesFrame, trainMax[i])

            validMinLimit = getDate_Index(daysFrame, datesFrame, validMin[i])
            validMaxLimit = getDate_Index(daysFrame, datesFrame, validMax[i])
        
            testMinLimit = getDate_Index(daysFrame, datesFrame, testMin[i])
            testMaxLimit = getDate_Index(daysFrame, datesFrame, testMax[i])

            train = TrendSlope(iteration = i, minLimit=trainMinLimit,maxLimit=trainMaxLimit, name = "Day" ,type = "train")
            train.writeFile()
            valid = TrendSlope(iteration = i, minLimit=validMinLimit,maxLimit=validMaxLimit, name = "Day" ,type = "valid")
            valid.writeFile()
            test  = TrendSlope(iteration = i, minLimit=testMinLimit,maxLimit=testMaxLimit, name = "Day" ,type = "test")
            test.writeFile()   
