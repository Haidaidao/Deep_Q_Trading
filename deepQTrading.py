...
class DeepQTrading:
    ...
    def run(self):
        ...
        while(self.currentStartingPoint+self.walkSize <= self.endingPoint):
            ...
            ensambleTrain=pd.DataFrame(index=self.dates[trainMinLimit:trainMaxLimit+1].loc[:,'Date'].drop_duplicates().tolist())
            ensambleValid=pd.DataFrame(index=self.dates[validMinLimit:validMaxLimit+1].loc[:,'Date'].drop_duplicates().tolist())
            ensambleTest=pd.DataFrame(index=self.dates[testMinLimit:testMaxLimit+1].loc[:,'Date'].drop_duplicates().tolist())
            ...
            ensambleTrain.to_csv("./Output/ensemble/"+"/walk"+str(iteration)+"ensemble_valid.csv")
