...
class Evaluation:
    def __init__(self, data, column, filename, initial_balance=1000, trading_cost_ratio=0.001):
        self.data = data.loc[:, ['close', column]]
    ...
    def evaluate(self):
        ...
        return self.current_balance, net_profit, self.winning_trade_number, self.losing_trade_number, profit_factor, percent_profitable, average_profit_per_trade
    ...
    def init(self):
        ...
        for i in range(len(self.data)):
            action = self.data.iloc[i][self.column]
            current_price = self.data.iloc[i]['close']
            if (action == -1 or action == 0) and num_shares == 0:
                # self.data.iloc[i, self.data.columns.get_loc('action')] = 'NONE'
                continue

            # Vào lệnh
            if action == 1 and num_shares == 0:  # buy và trả phí giao dịch
            ...
            elif action == -1 and num_shares > 0:  # then sell and pay the transaction cost
