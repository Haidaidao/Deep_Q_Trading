...
def ensemble_y_true_with_optimize_trend(feature, stats, threshold):
    ...
    for index, row in feature.iterrows():
        ensemble_value = row['ensemble']  # Lấy giá trị từ cột 'ensemble'
        ...
        if changes >= threshold or (last_action > 0 and changes >= 0 and changes < threshold):
            last_action = ensemble_value 
            action = ensemble_value 
...
def RandomForestEnsemble(numWalks,perc,type,numDel):
    ...
    columns = ["From","To", "Reward%", "#Wins", "#Losses", "Ratio" ,"Dollars", "Coverage", "Accuracy"]
